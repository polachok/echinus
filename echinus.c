...
setup(void)
{
	...
	XftColorAllocName(dpy, DefaultVisual(dpy, screen), DefaultColormap(dpy,
		screen), getresource("selected.fg", SELFGCOLOR), dc.xftsel);
	XftColorAllocName(dpy, DefaultVisual(dpy, screen), DefaultColormap(dpy,
		screen), getresource("normal.fg", SELFGCOLOR), dc.xftnorm);
...
view(const char *arg)
{
	...
	memcpy(cm->prevtags, cm->seltags, ntags * sizeof(cm->seltags));
	...
	for (m = monitors; m; m = m->next) {
		if (m->seltags[i] && m != cm) {
			memcpy(m->prevtags, m->seltags, ntags * sizeof(m->seltags));
			memcpy(m->seltags, cm->prevtags, ntags * sizeof(cm->seltags));
			updategeom(m);
			arrange(m);
		}
	}
